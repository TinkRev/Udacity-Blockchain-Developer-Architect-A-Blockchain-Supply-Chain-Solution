...
contract SupplyChain is ConsumerRole,DistributorRole,FarmerRole,RetailerRole {
  ...
  // and set 'sku' to 1
  // and set 'upc' to 1
  constructor() FarmerRole() DistributorRole() RetailerRole() ConsumerRole() public payable {
    owner = msg.sender;
    sku = 1;
    upc = 1;
  }
  ...
  // Call modifier to check if upc has passed previous supply chain stage: previous stage is Harvested
  
  // Call modifier to verify caller of this function: current owiner is Farmer in Harvested stage
  ...
  function buyItem(uint _upc) public payable onlyDistributor() forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc)
    ...
    Item memory tmp = items[_upc];
    tmp.ownerID = msg.sender;
    tmp.distributorID = msg.sender;
    tmp.itemState = State.Sold;
    items[_upc] = tmp;
    // Transfer money to farmer
    address(uint160(items[_upc].originFarmerID)).transfer(tmp.productPrice);
    // emit the appropriate event
    emit Sold(_upc);
  }
  ...
  function receiveItem(uint _upc) public onlyRetailer() shipped(_upc)
    ...
    Item memory tmp = items[_upc];
    tmp.ownerID = msg.sender;
    tmp.retailerID = msg.sender;
    tmp.itemState = State.Received;
    items[_upc] = tmp;
    // Emit the appropriate event
    emit Received(_upc);
  }
  ...
  function purchaseItem(uint _upc) public onlyConsumer() received(_upc)
    ...
    Item memory tmp = items[_upc];
    tmp.ownerID = msg.sender;
    tmp.consumerID = msg.sender;
    tmp.itemState = State.Purchased;
    items[_upc] = tmp;
    // Emit the appropriate event
    emit Purchased(_upc);
  }
  ...
  // Assign values to the 8 parameters
    ...
    itemUPC = tmp.upc;
    ownerID = tmp.ownerID;
    originFarmerID = tmp.originFarmerID;
    originFarmName = tmp.originFarmName;
    originFarmInformation = tmp.originFarmInformation;
    originFarmLatitude = tmp.originFarmLatitude;
    originFarmLongitude = tmp.originFarmLongitude;
    ...
    ) 
  {
    ...
    productNotes = tmp.productNotes;
    productPrice = tmp.productPrice;
    itemState = uint(tmp.itemState);
    distributorID = tmp.distributorID;
    retailerID = tmp.retailerID;
    consumerID = tmp.consumerID;
  return 
  (
  itemSKU,
  itemUPC,
